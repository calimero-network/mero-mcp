 ...
 * it. So authors write base-less links (`/reference/tools/`) and this rewrites
 * them to `/mero-mcp/reference/tools/` at request time (dev) and prerender time
 * (build).
 *
 * Skips links already under the base and protocol-relative (`//`) URLs.
 */
const BASE = '/mero-mcp'; // keep in sync with astro.config.mjs
