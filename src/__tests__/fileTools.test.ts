...
describe("File Tools", () => {
  ...
  describe("getFileInfo", () => {
    it("should get file info successfully", async () => {
      ...
      // Verify logging
      expect(logger.info).toHaveBeenCalledWith("File info retrieved", {
        path: "test.txt",
      });
    });
    ...
    it("should handle errors when getting file info", async () => {
      ...
      // Verify error handling
      expect(result.content[0].text).toContain(
        "Error getting file info for nonexistent.txt",
      );
      expect(result.content[0].text).toContain("File not found");
  ...
  describe("listDirectory", () => {
    ...
    it("should handle errors when listing a directory", async () => {
      ...
      // Verify error handling
      expect(result.content[0].text).toContain(
        "Error listing directory nonexistent-dir",
      );
  ...
  describe("writeFile", () => {
    it("should write a file successfully", async () => {
      ...
      // Verify logging
      expect(logger.info).toHaveBeenCalledWith("File written successfully", {
        path: "test.txt",
      });
    });
  ...
  describe("deleteFile", () => {
    ...
    it("should handle errors when deleting a file that does not exist", async () => {
      ...
      // Verify error handling
      expect(result.content[0].text).toContain(
        "Error deleting file nonexistent.txt",
      );
      expect(result.content[0].text).toContain("File not found");
